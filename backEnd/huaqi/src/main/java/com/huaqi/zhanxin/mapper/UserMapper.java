...
public interface UserMapper {
    ...
    @Update("update Identity_Info set occupation = #{occupation},annual_income = #{annualIncome},"
            +
            "working_years = #{workingYears},phone=#{phone} where user_id = #{userID}")
    int updateInfo(@Param("userID") int userID, @Param("occupation") int occupation,
                   @Param("annualIncome") float annualIncome, @Param("workingYears") int workingYears,
                   @Param("phone") String phone);

    @Insert("insert into Identity_Info(occupation,annual_income,working_years,user_id,phone) "
            +
            "values (#{occupation},#{annualIncome},#{workingYears},#{userID},#{phone})")
    int insertInfo(@Param("occupation") int occupation, @Param("annualIncome") float annualIncome,
                   @Param("workingYears") int workingYears, @Param("userID") int userID, @Param("phone") String phone);
    ...
    int registerUser(@Param("userEmail") String userEmail, @Param("userPwd") String userPwd,
                     @Param("userType") int userType, @Param("userRegisterTime") LocalDateTime userRegisterTime);

    //实名
    @Update("update Identity_Info set authentication = #{authentication},IDtype = #{IDtype},IDcard = #{IDcard} where user_id = #{userID}")
    int updateAuthentication(@Param("userID") int userID, @Param("authentication") boolean authentication,
                             @Param("IDtype") int IDtype, @Param("IDcard") String IDcard);
    ...
    @Insert("insert into Credit_Record values (#{userID},#{debtRatio},#{numberRealEstateLoansOrLines},"
            +
            "#{numberOfOpenCreditLinesAndLoans},#{numberOfTime30To59DaysPastDueNotWorse},"
            +
            "#{revolvingUtilizationOfUnsecuredLines},#{seriousDlqin2yrs},#{monthlyIncome},#{numberOfTime90DaysLate},"
            +
            "#{numberOfDependents},#{numberOfTime60To89DaysPastDueNotWorse})")
    int insertCreditRecord(@Param("userID") int userID, @Param("debtRatio") double debtRatio,
                           @Param("numberRealEstateLoansOrLines") int numberRealEstateLoansOrLines,
                           @Param("numberOfOpenCreditLinesAndLoans") int numberOfOpenCreditLinesAndLoans,
                           @Param("numberOfTime30To59DaysPastDueNotWorse") int numberOfTime30To59DaysPastDueNotWorse,
                           @Param("revolvingUtilizationOfUnsecuredLines") double revolvingUtilizationOfUnsecuredLines,
                           @Param("seriousDlqin2yrs") int seriousDlqin2yrs,
                           @Param("monthlyIncome") double monthlyIncome,
                           @Param("numberOfTime90DaysLate") int numberOfTime90DaysLate,
                           @Param("numberOfDependents") int numberOfDependents,
                           @Param("numberOfTime60To89DaysPastDueNotWorse") int numberOfTime60To89DaysPastDueNotWorse);
