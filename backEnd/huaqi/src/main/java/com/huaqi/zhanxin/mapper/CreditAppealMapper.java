...
public interface CreditAppealMapper {
    ...
    int insertAppeal(@Param("userID") int userID, @Param("appealTime") Timestamp appealTime,
                     @Param("reason") String reason, @Param("question") String question,
                     @Param("require") String require, @Param("appealState") int appealState);
    ...
    List<JSONObject> selectAllByType(@Param("type") String type, @Param("pageNum") Integer pageNum,
    ...
    @Update({"update Credit_Appeal set appeal_state=1 where user_id=#{id} and appeal_time=#{time}; " +
            "update ${type} set ${detail}=#{value} where user_id=#{id}"})
    Integer updateAppealStateToAccepted(@Param("id") Integer user_id, @Param("time") Timestamp appeal_time,
                                        @Param("type") String type, @Param("detail") String detail,
